<template>
  <div ref="wrap">
    <div :style="leftSwitch" v-if="isHorizontal&&isLeft" :class="leftSwitchClass" @click="leftSwitchClick">
      <slot name="left-switch"></slot>
    </div>
    <div :style="rightSwitch" v-if="isHorizontal&&isRight" :class="rightSwitchClass" @click="rightSwitchClick">
  ...
  export default {
    ...
    data () {
      return {
        ...
        isLeft:false,//是否显示左侧切换按钮
        isRight:this.data.length>1?true:false,//是否显示右侧切换按钮
        index:0//数组的当前下标位置
      }
    },
    props: {
    ...
    methods: {
      leftSwitchClick () {
        this.index-=1;
        if(this.index==this.confine.start)
        {
           this.index=0;
           this.isLeft=false;
        }
      ...
      rightSwitchClick () {
        ...
        if(this.index+this.confine.end==this.data.length)
        {
          this.isRight=false;
        }
