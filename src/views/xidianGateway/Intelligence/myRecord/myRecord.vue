<template>
    <div class="myRecord">
        <el-tabs class="main mt10" v-model="activeName" >
            <el-tab-pane label="热门咨询" name="first">
                <div class="record" >我的记录</div>
                <div class="content" v-for="(item, index) in smallMyDataInf" :key="index" @click="changeState(index)" :class="{active:index===applyStatus}">
                    <div class="figure">
                        <img class="lottie" :src="item.icon" alt="">
                        <span class="lottieName">{{item.lottieName}}</span>
                    </div>
                    <div class="details">
            ...
            <el-tab-pane label="热门建议" name="second">

            </el-tab-pane>
            <el-tab-pane label="热门投诉" name="third">
...
<script>
    ...
    export default {
        ...
        return{
            ...
            myDataInf:[
            {
                icon: require('@/assets/lottie.png'),
                lottieName: 'Lottie',
                title: '【功能建议】建议个人用户的实名认证信息应提供修改功能',
                status: '已实现',
                time: '2019-08-05',
                neirong:"大多数小公司在初次使用阿里云时，都是小范围尝试使用。所以，注册的用户大多都是以公司员工的个人身份进行的申请和认证，并陆续购买了ECS或相关服务，并提供公司使用。但随着公司业务的发展，在已经开通的个人账户名下已经申请了……"
            },
            {
                icon: require('@/assets/lottie.png'),
                lottieName: 'Lottie',
        ...
        created(){
            this.smallMyDataInf = this.myDataInf.slice(0,5)
        },
        methods:{
            // searchprocess(){
            ...
            showTitle() {
                ...
                this.smallMyDataInf = this.myDataInf
